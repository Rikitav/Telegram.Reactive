...
namespace Telegram.Reactive.Core.Descriptors
{
    ...
    public class  DescribedHandlerInfo
    {
        ...
        {
            if (HandlerLifetime.IsLitetimeEnded)
                throw new Exception();

            if (HandlerInstance is not IHandlerContainerFactory containerFactory)
                throw new Exception();

            try
            {
                HandlerContainer = containerFactory.CreateContainer(UpdateRouter.AwaitingProvider,  this);
                await HandlerInstance.ExecuteInternal(HandlerContainer, cancellationToken);
            }
